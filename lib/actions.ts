...
export async function assignPendingSkill(
  ...
  const { installCompanionSkill } = await import('@/lib/queries')

  // Install the skill on the chosen companion
  const result = await installCompanionSkill(user.id, companionId, skillId, skillName)
  if ('error' in result) return result

  // Remove from pending
  await query(
    `DELETE FROM pending_skills WHERE id = $1 AND user_id = $2`,
    [pendingSkillId, user.id],
  )
